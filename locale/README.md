...
```bash
docker compose -f local-uv.yml run --rm django python manage.py makemessages --all --no-location
```
...
```bash
docker compose -f local-uv.yml run --rm django python manage.py compilemessages
```
